...
func (m *SharingReason) AddTo(o fmd.Metadata) (fmd.Metadata, error) {
	...
	f, err := sharingreason.Open(m.Path)
	if err != nil {
		return nil, fmt.Errorf("Could not load field: %w", err)
	}
	obj.SharingReasons = append(obj.SharingReasons, f.SharingReason)
	return obj, nil
}

func NewSharingReason(path string) Deployable {
	return &SharingReason{
		CustomObjectComponent: CustomObjectComponent{
			BaseMetadata: BaseMetadata{
				Path:         path,
				deployedType: "SharingReason",
				dir:          "objects",
			},
		},
	}
